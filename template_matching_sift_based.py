...
def sift_flann_ransac_matching(main_image_path, template_image_path, lowes_ratio=0.75, min_match_count=5,
    ...
    - bounding_box (tuple): Bounding box coordinates of the matched region (min_x, min_y, max_x, max_y).
    """
    # Load the images
    ...
    return result_image, cropped_result, bounding_box
...
if __name__ == "__main__":
    ...
    # main_image_path = '../data/template_matching/main/map_2.tif'
    # template_image_path = './sat_img/old/cam1_resized.png'
    import time
    t = time.time()
    result_image = sift_flann_ransac_matching(main_image_path, template_image_path)

    print("Time excuted: ",time.time() - t)
    result_image = cv2.resize(result_image, (1280,720))
    cv2.imshow('Template to find', cv2.imread(template_image_path))
